...
import com.od.TestSplitter.Basic.InformationExtracter;
...
import com.od.TestSplitter.Combinatorial.CombinatorialClass;
...
public class RawClass {
    ...
    private CombinatorialClass combinatorialClass;
    ...
    public void processCombinatorial() {
        String classPathForSplit = pathToClassFile.replace(Config.projectPath, Config.projectSplittedPath);
        combinatorialClass = new CombinatorialClass(cu.clone(), classPathForSplit, className, splittedClass.getSplittedMethods());
        combinatorialClass.process();
    }
    ...
    public void writeAllToFileBasic() {
    ...
    public void writeAllToFileInterCombinatorial() {
        if (splittedClass == null || intermediateSystemClass == null || combinatorialClass == null)
            return;

        combinatorialClass.writeIntermediateCombinatorialClassToFile();
    }

    public void writeAllToFileCombinatorial() {
        if (splittedClass == null || intermediateSystemClass == null || combinatorialClass == null)
            return;

        combinatorialClass.writeCombinatorialClassToFile();
    }
}
